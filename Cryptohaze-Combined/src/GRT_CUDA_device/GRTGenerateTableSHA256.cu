...
// CUDA SHA256 kernels for table generation.
...
// Some CUDA variables
__device__ __constant__ unsigned char SHA256_Generate_Device_Charset_Constant[512]; // Constant space for charset
__device__ __constant__ uint32_t SHA256_Generate_Device_Charset_Length; // Character set length
__device__ __constant__ uint32_t SHA256_Generate_Device_Chain_Length; // May as well pull it from constant memory... faster.
__device__ __constant__ uint32_t SHA256_Generate_Device_Number_Of_Chains; // Same, may as well be constant.
...
#define CREATE_SHA256_GEN_KERNEL(length) \
__global__ void MakeSHA256ChainLen##length(unsigned char *InitialPasswordArray, unsigned char *OutputHashArray, \
    ...
    copySingleCharsetToShared(charset, SHA256_Generate_Device_Charset_Constant); \
    password_index = ((blockIdx.x*blockDim.x + threadIdx.x) + (PasswordSpaceOffset * SHA256_Generate_Device_Number_Of_Threads)); \
    if (password_index >= SHA256_Generate_Device_Number_Of_Chains) { \
        return; \
    } \
    ...
    for (PassCount = 0; PassCount < StepsToRun; PassCount++) { \
        ...
        charset_offset++; \
        if (charset_offset >= SHA256_Generate_Device_Charset_Length) { \
            charset_offset = 0; \
        } \
    } \
    if (CurrentStep >= (SHA256_Generate_Device_Chain_Length - 1)) { \
        OutputArray32[0 * SHA256_Generate_Device_Number_Of_Chains + password_index] = a; \
        OutputArray32[1 * SHA256_Generate_Device_Number_Of_Chains + password_index] = b; \
        OutputArray32[2 * SHA256_Generate_Device_Number_Of_Chains + password_index] = c; \
        OutputArray32[3 * SHA256_Generate_Device_Number_Of_Chains + password_index] = d; \
        OutputArray32[4 * SHA256_Generate_Device_Number_Of_Chains + password_index] = e; \
    } \
    else { \
...
CREATE_SHA256_GEN_KERNEL(6)
CREATE_SHA256_GEN_KERNEL(7)
CREATE_SHA256_GEN_KERNEL(8)
CREATE_SHA256_GEN_KERNEL(9)
CREATE_SHA256_GEN_KERNEL(10)




extern "C" void copyConstantsToSHA256(unsigned char *HOST_Charset, uint32_t HOST_Charset_Length,
    uint32_t HOST_Chain_Length, uint32_t HOST_Number_Of_Chains, uint32_t HOST_Table_Index,
    uint32_t HOST_Number_Of_Threads) {

    cudaMemcpyToSymbol(SHA256_Generate_Device_Charset_Constant,HOST_Charset, 512);
    cudaMemcpyToSymbol(SHA256_Generate_Device_Charset_Length, &HOST_Charset_Length, sizeof(uint32_t));

    // Copy general table parameters to constant space
    cudaMemcpyToSymbol(SHA256_Generate_Device_Chain_Length, &HOST_Chain_Length, sizeof(uint32_t));
    cudaMemcpyToSymbol(SHA256_Generate_Device_Number_Of_Chains, &HOST_Number_Of_Chains, sizeof(uint32_t));
    cudaMemcpyToSymbol(SHA256_Generate_Device_Table_Index, &HOST_Table_Index, sizeof(uint32_t));
    cudaMemcpyToSymbol(SHA256_Generate_Device_Number_Of_Threads, &HOST_Number_Of_Threads, sizeof(HOST_Number_Of_Threads));
}


extern "C" void LaunchGenerateKernelSHA256(int passwordLength, uint32_t CUDA_Blocks,
    ...
    switch (passwordLength) {
            ...
            case 8:
                ...
                break;
            case 9:
                MakeSHA256ChainLen9 <<< CUDA_Blocks, CUDA_Threads >>>
                    (DEVICE_Initial_Passwords, DEVICE_End_Hashes, PasswordSpaceOffset,
                    CurrentChainStartOffset, StepsPerInvocation, CharsetOffset);
                break;
            case 10:
                MakeSHA256ChainLen10 <<< CUDA_Blocks, CUDA_Threads >>>
                    (DEVICE_Initial_Passwords, DEVICE_End_Hashes, PasswordSpaceOffset,
                    CurrentChainStartOffset, StepsPerInvocation, CharsetOffset);
                break;
            default:
