...
__device__ __constant__ unsigned char SHA256_Regenerate_Device_Charset_Constant[512]; // Constant space for charset
__device__ __constant__ uint32_t SHA256_Regenerate_Device_Charset_Length; // Character set length
__device__ __constant__ uint32_t SHA256_Regenerate_Device_Chain_Length; // May as well pull it from constant memory... faster when cached.
__device__ __constant__ uint32_t SHA256_Regenerate_Device_Table_Index;
__device__ __constant__ uint32_t SHA256_Regenerate_Device_Number_Of_Threads; // It needs this, and can't easily calculate it
__device__ __constant__ uint32_t SHA256_Regenerate_Device_Number_Of_Chains_To_Regen;
__device__ __constant__ uint32_t SHA256_Regenerate_Device_Number_Of_Hashes;
...
extern "C" void copySHA256RegenerateDataToConstant(char *hostCharset, uint32_t hostCharsetLength,
    ...
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(SHA256_Regenerate_Device_Charset_Constant, hostCharset, 512));
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(SHA256_Regenerate_Device_Charset_Length, &hostCharsetLength, sizeof(uint32_t)));
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(SHA256_Regenerate_Device_Chain_Length, &hostChainLength, sizeof(uint32_t)));
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(SHA256_Regenerate_Device_Table_Index, &hostTableIndex, sizeof(uint32_t)));
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(SHA256_Regenerate_Device_Number_Of_Threads, &hostNumberOfThreads, sizeof(uint32_t)));
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(constantBitmap, hostBitmap, 8192));
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(SHA256_Regenerate_Device_Number_Of_Hashes, &hostNumberOfHashes, sizeof(uint32_t)));
}

extern "C" void setSHA256RegenerateNumberOfChains(uint32_t numberOfChains) {
    CH_CUDA_SAFE_CALL(cudaMemcpyToSymbol(SHA256_Regenerate_Device_Number_Of_Chains_To_Regen, &numberOfChains, sizeof(uint32_t)));
}
...
__global__ void RegenSHA256ChainLen##length(unsigned char *InitialPasswordArray, unsigned char *FoundPasswordArray, \
    ...
    copySingleCharsetToShared(charset, SHA256_Regenerate_Device_Charset_Constant); \
    copyBitmap(sharedBitmap); \
    password_index = ((blockIdx.x * blockDim.x + threadIdx.x) + (PasswordSpaceOffset * SHA256_Regenerate_Device_Number_Of_Threads)); \
    if (password_index >= SHA256_Regenerate_Device_Number_Of_Chains_To_Regen) { \
        return; \
    } \
    ...
    LoadMD5RegistersFromGlobalMemory(b0,b1,b2,b3,b4,b5,b6,b7,b8,b9,b10,b11,b12,b13,b14,b15, \
        InitialArray32, SHA256_Regenerate_Device_Number_Of_Chains_To_Regen, password_index, pass_length); \
    for (PassCount = 0; PassCount < StepsToRun; PassCount++) { \
        ...
        if ((sharedBitmap[(a & 0x0000ffff) >> 3] >> (a & 0x00000007)) & 0x00000001) { \
            { \
                search_high = SHA256_Regenerate_Device_Number_Of_Hashes; \
                search_low = 0; \
                search_index = 0; \
                while (search_low < search_high) { \
                    ...
                    if ((hash_order_a == hash_order_mem) && (search_low < SHA256_Regenerate_Device_Number_Of_Hashes)) { \
                        break; \
                    } \
                } \
...
next: \
        clearB0toB15(b0,b1,b2,b3,b4,b5,b6,b7,b8,b9,b10,b11,b12,b13,b14,b15); \
        reduceSingleCharsetNormal(b0, b1, b2, a, b, c, d, CurrentStep, charset, charset_offset, pass_length, SHA256_Regenerate_Device_Table_Index); \
        charset_offset++; \
        if (charset_offset >= SHA256_Regenerate_Device_Charset_Length) { \
            charset_offset = 0; \
        } \
    } \
    if (CurrentStep >= (SHA256_Regenerate_Device_Chain_Length - 1)) { \
    }  \
    else { \
...
CREATE_SHA256_REGEN_KERNEL(6)
CREATE_SHA256_REGEN_KERNEL(7)
CREATE_SHA256_REGEN_KERNEL(8)
CREATE_SHA256_REGEN_KERNEL(9)
CREATE_SHA256_REGEN_KERNEL(10)
...
extern "C" void LaunchSHA256RegenerateKernel(int PasswordLength, int CUDA_Blocks, int CUDA_Threads,
    ...
    switch (PasswordLength) {
        case 6:
            RegenSHA256ChainLen6 <<< CUDA_Blocks, CUDA_Threads >>>
            ...
            break;
        case 7:
            RegenSHA256ChainLen7 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
        case 8:
            RegenSHA256ChainLen8 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
        case 9:
            RegenSHA256ChainLen9 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
        case 10:
            RegenSHA256ChainLen10 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
