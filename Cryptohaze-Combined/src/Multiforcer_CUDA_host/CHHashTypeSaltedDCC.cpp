...
#include "Multiforcer_CUDA_host/CHHashTypeSaltedDCC.h"
#include "Multiforcer_Common/CHCommon.h"

// Call the constructor of CHHashTypeSalted with hash len 16, salt len 32
CHHashTypeSaltedDCC::CHHashTypeSaltedDCC() : CHHashTypeSalted(NTLM_HASH_LENGTH, 32) {

}


void CHHashTypeSaltedDCC::copyDataToConstant(char *hostCharset, int charsetLength,
        unsigned char *hostCharsetLengths, unsigned char *hostSharedBitmap, int threadId) {
    copyDCCDataToConstant(hostCharset, charsetLength, hostCharsetLengths,
            hostSharedBitmap, threadId, this->SaltList, this->SaltLengths, this->NumberOfHashes);
}

void CHHashTypeSaltedDCC::Launch_CUDA_Kernel(int passlength, uint64_t charsetLength, int numberOfPasswords, unsigned char *DEVICE_Passwords,
    ...
    Launch_CUDA_DCC_Kernel(passlength, charsetLength, numberOfPasswords,
