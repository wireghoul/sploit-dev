...
#if RELEASE_KERNEL
extern char MFNHashTypePlainOpenCL_SHA1_source[];
#endif

#define reverse(x)(x>>24)|((x<<8) & 0x00FF0000)|((x>>8) & 0x0000FF00)|(x<<24);


MFNHashTypePlainOpenCL_SHA1::MFNHashTypePlainOpenCL_SHA1() :  MFNHashTypePlainOpenCL(20) {
    trace_printf("MFNHashTypePlainOpenCL_SHA1::MFNHashTypePlainOpenCL_SHA1()\n");
    // Reverse the hash in the processing steps for hash matching.
    this->HashIsBigEndian = 1;
}

void MFNHashTypePlainOpenCL_SHA1::launchKernel() {
    trace_printf("MFNHashTypePlainOpenCL_SHA1::launchKernel()\n");
...
void MFNHashTypePlainOpenCL_SHA1::printLaunchDebugData() {
...
std::vector<uint8_t> MFNHashTypePlainOpenCL_SHA1::preProcessHash(std::vector<uint8_t> rawHash) {
    //trace_printf("MFNHashTypePlainOpenCL_SHA1::preProcessHash()\n");
...
std::vector<uint8_t> MFNHashTypePlainOpenCL_SHA1::postProcessHash(std::vector<uint8_t> processedHash) {
    trace_printf("MFNHashTypePlainOpenCL_SHA1::postProcessHash()\n");
...
void MFNHashTypePlainOpenCL_SHA1::copyConstantDataToDevice() {
    trace_printf("MFNHashTypePlainOpenCL_SHA1::copyConstantDataToDevice()\n");
...
std::vector<std::string> MFNHashTypePlainOpenCL_SHA1::getHashFileNames() {
    trace_printf("MFNHashTypePlainOpenCL_SHA1::getHashFileNames()\n");
    
    std::vector<std::string> returnHashFilenames;
    std::string hashFilename;
    
    hashFilename = "./src/MFN_OpenCL_device/MFNHashTypePlainOpenCL_SHA1.cl";
    returnHashFilenames.push_back(hashFilename);
    return returnHashFilenames;
}

std::string MFNHashTypePlainOpenCL_SHA1::getKernelSourceString() {
    std::string ReturnString;
    
#if RELEASE_KERNEL
    ReturnString += std::string(MFNHashTypePlainOpenCL_SHA1_source);
#endif
    
    return ReturnString;
}

std::string MFNHashTypePlainOpenCL_SHA1::getHashKernelName() {
    trace_printf("MFNHashTypePlainOpenCL_SHA1::getHashKernelName()\n");
    
    std::string hashKernel = "MFNHashTypePlainOpenCL_SHA1";
    return hashKernel;
}

std::string MFNHashTypePlainOpenCL_SHA1::getDefineStrings() {
    ...
    //additionalDefines += DefineGen.makeBitmapLookup(this->VectorWidth, numberBitmaps, "CheckPassword160");
    
    additionalDefines += 
        DefineGen.makeBitmapLookupEarlyOut(this->VectorWidth,  "CheckPassword160",
        'a', bitmapAValid, 
        'b', bitmapBValid, "",
        'c', bitmapCValid, "",
        'd', bitmapDValid, "",
