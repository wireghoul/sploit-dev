...
extern char MFNHashTypePlainOpenCL_MD5WL_source[];
#endif


MFNHashTypePlainOpenCL_MD5WL::MFNHashTypePlainOpenCL_MD5WL() :  MFNHashTypePlainOpenCL(16) {
    trace_printf("MFNHashTypePlainOpenCL_MD5WL::MFNHashTypePlainOpenCL_MD5WL()\n");
    
    this->hashAttributes.hashWordWidth32 = 1;
    this->hashAttributes.hashUsesWordlist = 1;
}

void MFNHashTypePlainOpenCL_MD5WL::launchKernel() {
    trace_printf("MFNHashTypePlainOpenCL_MD5WL::launchKernel()\n");
...
void MFNHashTypePlainOpenCL_MD5WL::copyConstantDataToDevice() {
    trace_printf("MFNHashTypePlainOpenCL_MD5WL::copyConstantDataToDevice()\n");
    ...
    for (int i = 0; i < this->HashKernelVector.size(); i++) {
        ...
        klaunch_printf("Setting args for kernel %d\n", i);
...
void MFNHashTypePlainOpenCL_MD5WL::copyWordlistSizeToDevice(cl_uint wordCount, cl_uchar blocksPerWord) {
    cl_int errorCode;
...
std::vector<std::string> MFNHashTypePlainOpenCL_MD5WL::getHashFileNames() {
    trace_printf("MFNHashTypePlainOpenCL_MD5WL::getHashFileNames()\n");
    ...
    returnHashFilenames.push_back(hashFilename);
    hashFilename = "./src/MFN_OpenCL_device/MFNHashTypePlainOpenCL_MD5WL.cl";
    returnHashFilenames.push_back(hashFilename);
#endif
    return returnHashFilenames;
}

std::string MFNHashTypePlainOpenCL_MD5WL::getKernelSourceString() {
    ...
    ReturnString += std::string(MFNHashTypePlainOpenCL_MD5WL_source);
#endif
...
std::vector<std::string> 
    MFNHashTypePlainOpenCL_MD5WL::getHashKernelNamesVector() {
    trace_printf("MFNHashTypePlainOpenCL_MD5WL::getHashKernelNamesVector()\n");
    
    std::vector<std::string> kernelNames;
    
    // Kernel 0: Block length 1-4
    kernelNames.push_back(std::string("MFNHashTypePlainOpenCL_MD5WL_B1_4"));

    // Kernel 1: Block length 5-8
    kernelNames.push_back(std::string("MFNHashTypePlainOpenCL_MD5WL_B5_8"));

    // Kernel 2: Block length 9-14
    kernelNames.push_back(std::string("MFNHashTypePlainOpenCL_MD5WL_B9_14"));

    // Kernel 3: Block length 15-16
    kernelNames.push_back(std::string("MFNHashTypePlainOpenCL_MD5WL_B15_16"));

    // Kernel 4: Block length 17-30
    kernelNames.push_back(std::string("MFNHashTypePlainOpenCL_MD5WL_B17_30"));

    // Kernel 5: Block length 31-32
    kernelNames.push_back(std::string("MFNHashTypePlainOpenCL_MD5WL_B31_32"));
    
    return kernelNames;
}

cl_kernel MFNHashTypePlainOpenCL_MD5WL::getKernelToRun() {
    ...
    switch (this->wordlistBlockLength) {
        ...
        case 3:
        case 4:
            // MFNHashTypePlainOpenCL_MD5WL_B1_4
        ...
        case 7:
        case 8:
            // MFNHashTypePlainOpenCL_MD5WL_B5_8
...
std::string MFNHashTypePlainOpenCL_MD5WL::getDefineStrings() {
