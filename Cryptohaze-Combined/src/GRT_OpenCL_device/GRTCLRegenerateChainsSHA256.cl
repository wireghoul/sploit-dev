...
void searchForHashSHA256(uint a, uint b, uint c, uint d, uint vectorPosition,
...
inline void searchForHashSHA256(uint a, uint b, uint c, uint d, uint vectorPosition,
        LOCAL_MEMORY_TYPE unsigned char *sharedBitmap,
        uint deviceNumberOfHashes, __global unsigned int *deviceHashArray,
        __global   unsigned char *foundPasswordArray, __global  unsigned char *deviceSuccessArray,
        uint b0, uint b1, uint b2, uint b3) {
        ...
        //printf("%08x %08x %08x %08x\n", a, b, c, d);

            {
                ...
                while (search_low < search_high) {
                    ...
                    temp = deviceHashArray[8 * search_index];
                ...
                while (search_index && (a == deviceHashArray[(search_index - 1) * 8])) {
                    search_index--;
                }
                while ((a == deviceHashArray[search_index * 8])) {
                    {
                        if (b == deviceHashArray[search_index * 8 + 1]) {
                            if (c == deviceHashArray[search_index * 8 + 2]) {
                                if (d == deviceHashArray[search_index * 8 + 3]) {
                                    #if CPU_DEBUG
                                    printf("HASH FOUND!\n");
                                    printf("%08x %08x\n", b0, b1);
                                    #endif
...
__kernel __attribute__((vec_type_hint(vector_type))) void RegenerateChainsSHA256(
    ...
    for (PassCount = 0; PassCount < deviceStepsToRun; PassCount++) {
        ...
        padSHA256Hash(PASSWORD_LENGTH, &b0, &b1, &b2, &b3, &b4, &b5, &b6, &b7, &b8, &b9, &b10, &b11, &b12, &b13, &b14, &b15);
        b0 = reverse(b0); b1 = reverse(b1); b2 = reverse(b2); b3 = reverse(b3);
        ...
        searchForHashSHA256(a.s0, b.s0, c.s0, d.s0, 0, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
                passb0.s0, passb1.s0, passb2.s0, passb3.s0);
#endif
#if grt_vector_2 || grt_vector_4 || grt_vector_8 || grt_vector_16
        searchForHashSHA256(a.s1, b.s1, c.s1, d.s1, 1, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
                passb0.s1, passb1.s1, passb2.s1, passb3.s1);
#endif
#if grt_vector_4 || grt_vector_8 || grt_vector_16
        searchForHashSHA256(a.s2, b.s2, c.s2, d.s2, 2, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
                passb0.s2, passb1.s2, passb2.s2, passb3.s2);
        searchForHashSHA256(a.s3, b.s3, c.s3, d.s3, 3, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
                passb0.s3, passb1.s3, passb2.s3, passb3.s3);
#endif
#if grt_vector_8 || grt_vector_16
        searchForHashSHA256(a.s4, b.s4, c.s4, d.s4, 4, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
                passb0.s4, passb1.s4, passb2.s4, passb3.s4);
        searchForHashSHA256(a.s5, b.s5, c.s5, d.s5, 5, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
                passb0.s5, passb1.s5, passb2.s5, passb3.s5);
        searchForHashSHA256(a.s6, b.s6, c.s6, d.s6, 6, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
                passb0.s6, passb1.s6, passb2.s6, passb3.s6);
        searchForHashSHA256(a.s7, b.s7, c.s7, d.s7, 7, BITMAP_NAME, deviceNumberOfHashes, deviceHashArray, foundPasswordArray, deviceSuccessArray,
