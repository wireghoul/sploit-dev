...
__kernel __attribute__((vec_type_hint(vector_type))) void CandidateHashSHA256(
    ...
    __constant unsigned int *SHA256_Candidate_Device_Hash,/* 5 */
    ...
    printf("startStep: %d\n", startStep);
    printf("stepsToRun: %d\n", stepsToRun);
    printf("Candidate hash: %08x %08x %08x %08x\n", SHA256_Candidate_Device_Hash[0],
            SHA256_Candidate_Device_Hash[1], SHA256_Candidate_Device_Hash[2], SHA256_Candidate_Device_Hash[3]);
    ...
    uint init_a, init_b, init_c, init_d, init_e, init_f, init_g, init_h;
    ...
    // it as needed.
    if (startStep == 0) {
        init_a = (uint)SHA256_Candidate_Device_Hash[0];
        init_b = (uint)SHA256_Candidate_Device_Hash[1];
        init_c = (uint)SHA256_Candidate_Device_Hash[2];
        init_d = (uint)SHA256_Candidate_Device_Hash[3];
        init_e = (uint)SHA256_Candidate_Device_Hash[4];
        init_f = (uint)SHA256_Candidate_Device_Hash[5];
        init_g = (uint)SHA256_Candidate_Device_Hash[6];
        init_h = (uint)SHA256_Candidate_Device_Hash[7];
#if CPU_DEBUG
    ...
    #if CPU_DEBUG
        printf("SHA256 result .s0: %08x %08x %08x %08x \n", a.s0, b.s0, c.s0, d.s0);
        printf("SHA256 result .s1: %08x %08x %08x %08x \n", a.s1, b.s1, c.s1, d.s1);
    #endif
        a.s1 = init_a;
        b.s1 = init_b;
        c.s1 = init_c;
