...
#error "Vector widths 8 & 16 are NOT SUPPORTED!";
#endif

__kernel __attribute__((vec_type_hint(vector_type))) void CandidateHashNTLMAMD(
    ...
    __constant unsigned int *NTLM_Candidate_Device_Hash,/* 5 */
    ...
    printf("startStep: %d\n", startStep);
    printf("stepsToRun: %d\n", stepsToRun);
    printf("Candidate hash: %08x %08x %08x %08x\n", NTLM_Candidate_Device_Hash[0],
            NTLM_Candidate_Device_Hash[1], NTLM_Candidate_Device_Hash[2], NTLM_Candidate_Device_Hash[3]);
    ...
    uint init_a, init_b, init_c, init_d;
    ...
    // it as needed.
    if (startStep == 0) {
        init_a = (uint)NTLM_Candidate_Device_Hash[0];
        init_b = (uint)NTLM_Candidate_Device_Hash[1];
        init_c = (uint)NTLM_Candidate_Device_Hash[2];
        init_d = (uint)NTLM_Candidate_Device_Hash[3];
#if CPU_DEBUG
        printf("Loaded candidate %08x %08x %08x %08x \n", init_a, init_b, init_c, init_d);
#endif
        a = (vector_type) 0;
        b = (vector_type) 0;
        c = (vector_type) 0;
    ...
    // Now step through the vectors to load them properly...
        ...
        c.s0 = init_c;
        d.s0 = init_d;
        reduceSingleCharsetNTLM(&b0, &b1, &b2, &b3, &b4, &b5, a, b, c, d, step_to_calculate, charset, charset_offset, PASSWORD_LENGTH, deviceTableIndex);
    #if CPU_DEBUG
...
#if CPU_DEBUG
    printf("Loaded hash from location %d .s0:  %08x %08x %08x %08x \n", chain_index,
            a.s0, b.s0, c.s0, d.s0);
    printf("Loaded hash from location %d .s1:  %08x %08x %08x %08x \n", chain_index,
            a.s1, b.s1, c.s1, d.s1);
#endif
    ...
    printf("Chain step %d \n", i);
        ...
        OpenCL_MD4(b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12, b13, b14, b15, &a, &b, &c, &d);
#if CPU_DEBUG
    printf("NTLM result .s0: %08x %08x %08x %08x \n", a.s0, b.s0, c.s0, d.s0);
    printf("NTLM result .s1: %08x %08x %08x %08x \n", a.s1, b.s1, c.s1, d.s1);
#endif
        reduceSingleCharsetNTLM(&b0, &b1, &b2, &b3, &b4, &b5, a, b, c, d, i, charset, charset_offset, PASSWORD_LENGTH, deviceTableIndex);
#if CPU_DEBUG
    printf("Reduced password .s0: %08x %08x %08x\n", b0.s0, b1.s0, b2.s0);
    printf("Reduced password .s1: %08x %08x %08x\n", b0.s1, b1.s1, b2.s1);
#endif
