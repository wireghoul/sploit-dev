...
Copyright (C) 2013  Bitweasil (http://www.cryptohaze.com/)
...
/**
 * @section DESCRIPTION
 * CHHashFile is a base class for the hash file types.  They are a rewrite
 * of the CHHashFileV vector based classes with a focus on consolidation of
 * functionality to reduce duplicated code.
 * 
 * Everything is vector based, and all the network related data is cached to
 * improve performance under heavy loads when there are a lot of systems.  In
 * addition, the goal here is to remove all duplicate code in places, by storing
 * the original hash instead of regenerating it when needed.
 *
 * The network aspects of the code are handled by importing and exporting from
 * protobufs, and this code does not need to know anything about the network
 * layer.
 * 
 * As salted and iterated hashes are increasingly common, some of the caches
 * for them are defined here such that the common code can deal with them.
 */

#ifndef _CHHASHFILE_H
#define _CHHASHFILE_H
 ...
 * data return.  The salted file types will return this, but as it is a standard
 * function, it is defined here.
 */
typedef struct CHHashFileSaltedDataBlob {
    // The salt data, in "ready for hashing" form - convert hex to binary first.
    std::vector<std::vector<uint8_t> > SaltData;
    // Iteration count for iterated algorithms.
    std::vector<uint32_t> iterationCount;
    // Blobs of data that may be needed in addition (or instead of) the salt.
...
protected:
    ...
    /**
     * File for found hashes.
     * 
     * This is the opened file for hash output.  If the file is not being used,
     * this is set to NULL, otherwise the found hashes should be appended to
     * this file when reported.
     */
    FILE *OutputFile;
     ...
     * multi-algorithm kernel, or if you're running attacks against a number of
     ...
     * are sent to the rest of the classes.
     */
    uint8_t saltPrehashAlgorithm;
    
    // For use by all the classes if needed.
    CHHashImplementationMD5 HashImplementationMD5;
    
    // ============ Caching Variables ============
    /**
     * In order to improve performance with large numbers of clients over the
     * network, the hash and salt export protobufs are cached after being
     * generated.  This cache is valid until a hash is successfully found, at
     * which point the caches are invalidated and will be regenerated.  Because
     * both correspond to the same data set, there is only one "cache valid"
     * variable.  There is also a mutex used to protect the caches such that
     * only one thread can be in them at once.
     */
    
    // Set to true if the caches are valid, else false.
    uint8_t protobufExportCachesValid;
    // Cache for the hash data export.
    std::string hashExportProtobufCache;
    // Cache for the salt data export.
    std::string saltExportProtobufCache;
    
    /**
     * Clear the protobuf export caches.  This MUST be called when the data
     * regarding the found/not found hashes is updated (hashes added or
     * passwords found).
     */
    virtual void clearProtobufCache() {
        this->protobufExportCachesValid = 0;
        this->hashExportProtobufCache.clear();
        this->saltExportProtobufCache.clear();
    }
     ...
     */
    virtual int outputNewFoundHashesToFile() = 0;
     ...
     * @return A vector consisting of the binary value of the string.
     */
    virtual std::vector<uint8_t> convertAsciiToBinary(
        std::vector<char> asciiHex);
    ...
    /**
     * phpass encode/decode functions.  These are used in the phpass library
     * which is used by phpbb/Wordpress/etc.
     * 
    ...
    /**
     * Parse a line of the file and insert the data into the structure.
     * 
     * This is called by the main file reading function on each line of the
     * file.  It will do what is needed to understand the line and insert it
     * into the proper place.
     * 
     * @param fileLine The file line, stripped of any trailing newlines.
     * @param fileLine The line number of the file for reporting errors.
     */
    virtual void parseFileLine(std::string fileLine, size_t lineNumber) = 0;
    
    /**
     * Perform whatever post-loading operations need to happen.  This likely
     * involves sorting the hashes, extracting the uncracked unique salts,
     * and perhaps populating the protobuf cache.  Default is to do nothing.
     * 
     * This function must also set the number of total hashes/uncracked hashes
...
public:
    ...
    /**
     * Generate the actual protobuf to export and cache it.
     * 
     * This is called from exportHashListToRemoteSystem if the cache is not
     * valid.
     */
    virtual void createHashListExportProtobuf() = 0;
    ...
    /**
     * Generates the unique salt cached protobuf to export.  Not needed for
     * unsalted types, so default is a noop.
     */
