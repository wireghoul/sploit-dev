...
if (($argc < 5) || ($argc > 7)) {
    print "Usage: makeWordlistTest.php [hash name] [number passwords] [max length]\n";
    print "  [filename prefix] [opt: max salt length] [opt: charset flags]\n";

    // Please keep this updated...
    print "Supported hashes: MD5 NTLM IPB PHPASS\n";
    exit;
}
...
if ($argc == 7) {
    $charset = "";
    // Parse argv[6] and create the charset.
    if (strpos($argv[6], "?u") !== FALSE) {
        $charset .= "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    }
    if (strpos($argv[6], "?l") !== FALSE) {
        $charset .= "abcdefghijklmnopqrstuvwxyz";
    }
    if (strpos($argv[6], "?d") !== FALSE) {
        $charset .= "0123456789";
    }
    if (strpos($argv[6], "?s") !== FALSE) {
        $charset .= " !\"#$%&'()*+,-./:;<=>?@[\]^_`{|}~";
    }
    if (strpos($argv[6], "?1") !== FALSE) {
        $charset .= "1";
    }
    if (strpos($argv[6], "?2") !== FALSE) {
        $charset .= "2";
    }
    if (strlen($charset) == 0) {
...
switch ($argv[1]) {
    case "MD5":
        generateMD5TestWordlistHashes($argv[2], $argv[3], $charset, $argv[4]);
        break;
    case "NTLM":
        generateNTLMTestWordlistHashes($argv[2], $argv[3], $charset, $argv[4]);
        break;
    case "IPB":
        generateIPBTestWordlistHashes($argv[2], $argv[3], $charset, $argv[4]);
        break;
    case "PHPASS":
        generatePhpassTestWordlistHashes($argv[2], $argv[3], $charset, $argv[4]);
        break;
    default:
...
function generateMD5TestWordlistHashes($numPasswords, $passLength, $charset, $prefix) {
    ...
    for ($pl = 0; $pl < $passLength; $pl++) {
        ...
        $wordlistContents .= $passString . "\n";
        $hashlistContents .= md5($passString) . "\n";
    }
...
function generateIPBTestWordlistHashes($numPasswords, $passLength, $charset, $prefix) {
    // Generate one password for every password up to passLength.
    $wordlistContents = "";
    $hashlistContents = "";
    ...
    for ($pl = 0; $pl < $passLength; $pl++) {
        ...
        $wordlistContents .= $passString . "\n";
        $hashlistContents .= md5(md5($saltString).md5($passString)) . ":" . $saltString . "\n";
    }
...
function generatePhpassTestWordlistHashes($numPasswords, $passLength, $charset, $prefix) {
    // Generate one password for every password up to passLength.
    $wordlistContents = "";
    $hashlistContents = "";
    ...
    for ($pl = 0; $pl < $passLength; $pl++) {
        ...
        $wordlistContents .= $passString . "\n";
        $hashlistContents .= phpbb_hash($passString) . "\n";
    }
